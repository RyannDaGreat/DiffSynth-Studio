...
def launch_training_task(
    ...
    if os.environ.get("ACCELERATE_USE_DEEPSPEED", "false").lower() == "true":
        ...
        zero_stage = 3 if os.environ.get("USE_ZERO3", "false").lower() == "true" else 2
        config_file = f"deepspeed_config_{'zero3' if zero_stage == 3 else 'optimal'}.json"

        debug_print(f"[DeepSpeed] ZeRO-{zero_stage} | Config: {config_file} | Offload: CPU | grad_accum: {gradient_accumulation_steps}")

        # Disable CPU offload due to CUDA version mismatch issue
