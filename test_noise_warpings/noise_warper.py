...
def main(video:str,output_folder=None):
    ...
    first_frame_then_flow = output_folder.startswith('first_frame_then_')
    if first_frame_then_flow:
        rp.fansi_print(f"SKIPPING FIRSTFRAME IN THE FLOW! We will have {len(images)} flow frames in total.","italic bold yellow green on black black")
        images=images[1:]
